...
class decisionTreeThreeVariables():

    def __init__(self, n_episodes=100, min_epsilon=0.01, gamma=0.9, ada_divisor=10):
        self.n_episodes = n_episodes # training episodes
        self.features = ["Face Expression Recognition", "Object State", "Environmental Sound"]
        ...
        self.num_run = 1 # how many times you want to run the algorithm from scratch
    ...
    def initializeBuffer(self):
        ...
        while(i < self.MAX_LEN):
            ...
            if ("Object State" not in self.features):
                ...
                tuplex = tuple(listx)
                current_state = tuplex
            if (("Speech Emotion Recognition" in self.features) and ("Object State" in self.features)):
    ...
    def buildTree(self):
        ...
        total_importance = [0.0, 0.0, 0.0] # it considers the importance of all runs of the algorithm
        ...
        print("MONTECARLO ERROR CONSIDERING ALL FEATURES")
        m, inf, sup = self.mean_confidence_interval(total_error)
        print("Inferior interval: " + str(inf) + " Average: " + str(m) + " Superior Interval: " + str(sup))




        x = ["Tree without FER", "Tree without SER", "Tree without ES"]
        y = []
        e = []
        for i in range(0, len(self.features)):
        ...
        plt.xlabel("Trees considering two features")
